...
| **swipe-actionable row** | the gesture a phone uses where a desktop uses a right-click — Hormiga's table view | **built** (2026-09-25: it had never worked in a list of more than one. `SwipeListState` holds one offset for the whole list, and every row eased it toward its own rest, so the rows after the dragged one reset it each frame. One row owns the offset now: the open one, else the last dragged. A tap outside the open row closes it, as the header always said) |
| **touch scrolling** | a finger has no wheel, so no list on a phone scrolled at all (Void Hormiga's author: "this is huge") | **built 2026-09-25**: `touch_scroll` in `mobile.hpp`, once a frame after `NewFrame`. The first ~8 dp decide: a drag mostly along an axis the window under the finger (or its nearest scrollable parent, the wheel's rule) can scroll becomes a scroll, and the press is cancelled (`ClearActiveID`), so the card it started on never taps. Anything else (a sideways swipe row, the canvas) is left alone. Let go and it glides, decaying over about a second |
| **a finger pans the canvas** | a phone has no middle button, no Alt key and no wheel, so a graph on glass could not be moved | **built 2026-09-25**: with `CanvasStyle::touch`, a drag that starts on empty canvas pans (a tap there still deselects); the marquee stays one Shift away. Pinch-zoom is still the host's (Void Hormiga's phone Antfarm has zoom buttons) |
...
| **fling / momentum** | reported as a `Fling` event with velocity; the canvas does not yet coast, because inertia is an animation and `CanvasFx` is the place for it | **half**: recognized, not consumed by the canvas. Lists glide since 2026-09-25 (`touch_scroll`) |
